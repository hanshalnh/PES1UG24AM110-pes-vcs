...
int tree_from_index(ObjectID *id_out) {
    ...
    IndexEntry **entries = malloc(idx.count * sizeof(IndexEntry *));
    if (!entries) return -1;
    for (int i = 0; i < idx.count; i++) entries[i] = &idx.entries[i];
    int ret = write_tree_level(entries, idx.count, "", id_out);
    free(entries);
    return ret;
}
